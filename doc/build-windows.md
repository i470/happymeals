...
Below are some notes on how to build Axiom Core for Windows.

The options known to work for building Axiom Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Axiom Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Axiom Core applications.
...
Note that for WSL the Axiom Core source path MUST be somewhere in the default mount file system, for
example /usr/src/axiom, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/axiom/axiom.git
    sudo chmod -R a+rw axiom
...
way. This will install to `c:\workspace\axiom`, for example:

    make install DESTDIR=/mnt/c/workspace/axiom
...
causes two of the axiom executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the axiom code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the axiom source code).
