...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "axiom.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "axiomd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the AXM and zAXM money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 35433, 35435));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in AXM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in AXM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, axiom, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in AXM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Axiom specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:35433"));
    ...
    strUsage += HelpMessageOpt("-backupzaxm=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAxm minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeaxiomamount=<n>     " + strprintf(_("Keep N AXM anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           // FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Axiom Core Developers"), COPYRIGHT_YEAR)) + "\n" + // to be used instead of previous line from 2019
           // "\n" +                                                                                                     // to be used instead of previous line from 2019
           FormatParagraph(strprintf(_("Copyright (C) %i The Axiom Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Axiom is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Axiom Core is shutting down."));
...
#endif
    // Make sure only a single Axiom process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Axiom Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Axiom specific: zerocoin and spork DB's
                ...
                // Axiom: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZAXMMinted();
                        RecalculateZAXMSpent();
                    }
                    RecalculateAXMSupply(1);
                }
                ...
                // Axiom: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Axiom Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Axiom Core to complete") << "\n";
        ...
        bool fEnableZAxmBackups = GetBoolArg("-backupzaxm", true);
        pwalletMain->setZAxmAutoBackups(fEnableZAxmBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeAxiomAmount = GetArg("-anonymizeaxiomamount", 0);
//    if (nAnonymizeAxiomAmount > 999999) nAnonymizeAxiomAmount = 999999;
//    if (nAnonymizeAxiomAmount < 2) nAnonymizeAxiomAmount = 2;
    ...
    LogPrintf("Anonymize Axiom Amount %d\n", nAnonymizeAxiomAmount);
    ...
    /* Denominations
       ...
       For example:
       1AXM+1000 == (.1AXM+100)*10
       10AXM+10000 == (1AXM+1000)*10
    */
