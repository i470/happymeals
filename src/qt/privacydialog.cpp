                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAXM ought to be enough for anybody." - Bill Gates, 2017
    ui->zAXMpayAmount->setValidator( new QDoubleValidator(0.0, 398360470.0, 20, this) );
    ...
    ui->labelzAXMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAXM->setEnabled(false);
        ui->pushButtonMintzAXM->setToolTip(tr("zAXM is currently disabled due to maintenance."));

        ui->pushButtonSpendzAXM->setEnabled(false);
        ui->pushButtonSpendzAXM->setToolTip(tr("zAXM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAXMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAXM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAXM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zAXM, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAXM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAXM in ") +
...
void PrivacyDialog::on_pushButtonSpendzAXM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAXM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAXM
        sendzAXM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAXM
    sendzAXM();
}

void PrivacyDialog::on_pushButtonZAxmControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZAxmControlDialog* zAxmControl = new ZAxmControlDialog(this);
    zAxmControl->setModel(walletModel);
    zAxmControl->exec();
}

void PrivacyDialog::setZAxmControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAxmSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAXM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Axiom Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAXMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAXM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AXM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAXM</b>";
    ...
    // use mints from zAxm selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAxmControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAxmControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zAxm was spent successfully update the addressbook with the label
    ...
    // Clear zaxm selector in case it was used
    ZAxmControlDialog::listSelectedMints.clear();
    ui->labelzAxmSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAxm Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Axm, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAxm Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAXM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAXM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAXM "));
    ui->labelzAXMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAXM </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zAXM </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzAXMSyncStatus->setVisible(fShow);
}
