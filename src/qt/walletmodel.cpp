...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered axiom address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AXM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal axiom:URI (axiom:XyZ...?message=example)
            {
...
static void NotifyzAXMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAXMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAXMReset.connect(boost::bind(NotifyzAXMReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAXMReset.disconnect(boost::bind(NotifyzAXMReset, this));
}
