...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AXM);
    ...
    if (!settings.contains("nAnonymizeAxiomAmount"))
        settings.setValue("nAnonymizeAxiomAmount", 1000);

    nAnonymizeAxiomAmount = settings.value("nAnonymizeAxiomAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAxiomAmount"))
        SoftSetArg("-anonymizeaxiomamount", settings.value("nAnonymizeAxiomAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in axiom.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAxiomAmount:
            return QVariant(nAnonymizeAxiomAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAxiomAmount:
            nAnonymizeAxiomAmount = value.toInt();
            settings.setValue("nAnonymizeAxiomAmount", nAnonymizeAxiomAmount);
            emit anonymizeAxiomAmountChanged(nAnonymizeAxiomAmount);
            break;
        case CoinControlFeatures:
