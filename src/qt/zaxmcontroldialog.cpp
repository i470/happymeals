...
#include "ui_zaxmcontroldialog.h"
...
std::list<std::string> ZAxmControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAxmControlDialog::listMints;

ZAxmControlDialog::ZAxmControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAxmControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAxmControlDialog::~ZAxmControlDialog()
{
    delete ui;
}

void ZAxmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAxmControlDialog::updateList()
{
...
void ZAxmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAxmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAxm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAxmControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAxmControlDialog::GetSelectedMints()
{
...
void ZAxmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
