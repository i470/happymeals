...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 AXM at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAXM send
enum ZerocoinSpendStatus {
    ZAXM_SPEND_OKAY = 0,                            // No error
    ZAXM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAXM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAXM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAXM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAXM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAXM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAXM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAXM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAXM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAXM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAXM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAXM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAXM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAXM_SPENT_USED_ZAXM = 14,                      // Coin has already been spend
    ZAXM_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZAxmAutoBackups(bool fEnabled)
    {
    ...
    /** zAXM reset */
    boost::signals2::signal<void()> NotifyzAXMReset;
};
