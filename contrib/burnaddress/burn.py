...
Call the program with a template burn address as the only argument::
    % burn-btc AxiomBurnAddressBurnPremineXXXXXX
...
An example template is accessible using "test" as the argument::
    % burn-btc test
    AxiomBurnAddressBurnPremineXmtXTL
Validate bitcoin burn addresses at https://explorer.axiom.com/address/
"""
...
if __name__ == "__main__":
  if len(sys.argv) != 2:
    usage()
  if sys.argv[1] == "test":
    template = "AxiomBurnAddressBurnPremineXXXXXX"
  else:
    template = sys.argv[1]
  for c in template:
    if c not in ABET:
